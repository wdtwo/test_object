...
var datePicker = function(obj){
    ...
    //开始时间和结束时间
    let dateStart = obj.dateStart || undefined;
    let dateEnd = obj.dateEnd || undefined;
    ...
    function setNowDate(num){
        ...
        //前一个月
        if(num < 0 && today.startOf('month').valueOf() > dayjs(obj.dateStart).startOf('month').valueOf()){
        ...
        //翻页按钮是否可以点击
        if(today.startOf('month').valueOf() > dayjs(obj.dateStart).startOf('month').valueOf()){
            leftBtn.setAttribute('class',"btn-left")
        }else{
            leftBtn.setAttribute('class',"btn-left disabled")
        }
        //翻页按钮是否可以点击
        if(today.startOf('month').valueOf() < dayjs(obj.dateEnd).startOf('month').valueOf()){
            rightBtn.setAttribute('class',"btn-right")
        }else{
